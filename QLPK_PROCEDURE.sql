...
BEGIN
  ...
  INSERT_HOADON(100, 100, NULL, 'Chua thanh toán', 0, NULL, MAHD_OUT => v_MAHDOld);
...
CREATE OR REPLACE PROCEDURE QUANLYPHONGKHAM3.INSERT_DONTHUOC_HOADON (
...
BEGIN
  ...
  INSERT_HOADON(PAR_MALT, 3, NULL, 'Chưa thanh toán', 0, NULL, MAHD_OUT => v_MAHDOld);
...
CREATE OR REPLACE PROCEDURE INSERT_LOTHUOC (
  ...
  PAR_GIANHAP LOTHUOC.GIANHAP%TYPE, PAR_GIABAN LOTHUOC.GIABAN%TYPE, PAR_NGAYNHAP LOTHUOC.NGAYNHAP%TYPE, PAR_TRANGTHAI LOTHUOC.TRANGTHAI%TYPE)

AS
BEGIN

  INSERT INTO LOTHUOC (MALOTHUOC, MATHUOC, NHACC, SOLUONGTON, HANSD, GIANHAP, NGAYNHAP, GIABAN, TRANGTHAI)
  VALUES (SEQ15_MALT.NEXTVAL, PAR_MATHUOC, PAR_NHACC, PAR_SOLUONGTON, PAR_HANSD, PAR_GIANHAP, PAR_NGAYNHAP, PAR_GIABAN, PAR_TRANGTHAI);
...
CREATE OR REPLACE PROCEDURE UPDATE_LOTHUOC (
  PAR_MALOTHUOC LOTHUOC.MALOTHUOC%TYPE, PAR_NHACC LOTHUOC.NHACC%TYPE, 
  PAR_GIANHAP LOTHUOC.GIANHAP%TYPE,
...
BEGIN
  ...
  IF VAR_TENTHUOC IS NOT NULL THEN
    RAISE_APPLICATION_ERROR(-20001, 'Lo?i thu?c dã t?n t?i trong h? th?ng.');
  END IF;
