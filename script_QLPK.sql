...
CREATE SEQUENCE SEQ16_MACTDT START WITH 10;
...
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (3, '/benhnhan/:id', 'lay mot benh nhan khi biet id');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (4, '/bacsi/getAll', 'lay danh sach bac si');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (5, '/dichvu/getAll', 'lay danh sach dich vu');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (6, '/phieukham/insert-pk', 'them phieu kham');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (7, '/phieukham/insert-bn-pk', 'them benh nhan va phieu kham');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (8, '/phieukham/insert-just-pk', 'them chi phieu kham');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (9, '/phieukham/dsdk', 'lay danh sach dang ki');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (10, '/phieukham/chitiet-pk/:id', 'lay chi tiet cua mot phieu kham');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (11, '/phieukham/dspk/:id', 'lay chi tiet cua mot phieu kham');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (12, '/phieukham/ds-benh/:id', 'lay benh cua phieu kham');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (13, '/donthuoc/ds-thuoc/:id', 'lay thuoc cua mot don thuoc');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (14, '/hoadon/insert', 'them hoa don');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (15, '/hoadon/dshd/:id', 'them hoa don');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (16, '/cls/ds-cls/:id', 'them hoa don');
INSERT INTO VAITRO (MAVAITRO, URL, MOTA) VALUES (17, '/cls/getAll', 'them hoa don');
COMMIT;
...
INSERT INTO NHOM_VAITRO (MANHOM, MAVAITRO) VALUES (1, 13);
INSERT INTO NHOM_VAITRO (MANHOM, MAVAITRO) VALUES (1, 14);
INSERT INTO NHOM_VAITRO (MANHOM, MAVAITRO) VALUES (1, 15);
INSERT INTO NHOM_VAITRO (MANHOM, MAVAITRO) VALUES (1, 16);
INSERT INTO NHOM_VAITRO (MANHOM, MAVAITRO) VALUES (1, 17);
COMMIT;
...
-- GIÁ ĐƠN THUỐC THAY ĐỔI THEO THỜI GIAN ??
-- Giá đơn thuốc bằng tổng giá của các chi tiết đơn thuốc
CREATE OR REPLACE TRIGGER TRIGGER_UPDATE_GIADONTHUOC
AFTER INSERT OR UPDATE ON CTDT
FOR EACH ROW
DECLARE
    VAR_GIADONTHUOC NUMBER;
    VAR_GIATHUOC NUMBER;
BEGIN 

    SELECT DISTINCT L.GIABAN INTO VAR_GIATHUOC
    FROM LOTHUOC L
    JOIN THUOC T ON T.MATHUOC = L. MATHUOC
    WHERE T.MATHUOC = :NEW.MATHUOC;

    -- Cập nhật giá đơn thuốc cho đơn thuốc tương ứng
    UPDATE DONTHUOC
    SET GIADT = (:NEW.SOLUONGTHUOC * VAR_GIATHUOC)
    WHERE MADT = :NEW.MADT;
END;
/
...
CREATE OR REPLACE TRIGGER TRIGGER_KEDONTHUOC
BEFORE INSERT ON CTDT
FOR EACH ROW
DECLARE 

  VAR_SOLUONGTHUOC NUMBER;
  VAR_SOTHUOCDALAY NUMBER;
...
BEGIN
  ...
  FOR CUR_LOTHUOC IN (SELECT MALOTHUOC, SOLUONGTON 
                      FROM LOTHUOC 
                      WHERE MATHUOC = :NEW.MATHUOC AND SOLUONGTON > 0 
                      ORDER BY HANSD) 
  LOOP
  ...
  IF VAR_SOLUONGTHUOC > 0 THEN
    RAISE_APPLICATION_ERROR(-20001, 'Số lượng thuốc cần kê vượt quá số lượng tồn của tất cả các lô trong kho.');
  END IF;
  
END;
/

CREATE OR REPLACE TRIGGER TRIGGER_UPDATE_KEDONTHUOC
AFTER UPDATE ON CTDT
FOR EACH ROW
DECLARE 
  VAR_SOLUONGTHUOC_TRUOC NUMBER;
  VAR_SOLUONGTHUOC_SAU NUMBER;
  VAR_SOTHUOCDALAY NUMBER;
  VAR_CHENHLECH NUMBER;
  VAR_CAPNHAT NUMBER;
BEGIN
  VAR_SOTHUOCDALAY := 0;

  VAR_SOLUONGTHUOC_TRUOC := :OLD.SOLUONGTHUOC;
  VAR_SOLUONGTHUOC_SAU := :NEW.SOLUONGTHUOC;

  VAR_CHENHLECH := VAR_SOLUONGTHUOC_SAU - VAR_SOLUONGTHUOC_TRUOC;

  IF VAR_CHENHLECH >= 0 THEN
    ...
    LOOP
      IF VAR_CHENHLECH > 0 THEN
        IF VAR_CHENHLECH >= CUR_LOTHUOC.SOLUONGTON THEN
          VAR_SOTHUOCDALAY := VAR_SOTHUOCDALAY + CUR_LOTHUOC.SOLUONGTON;
          VAR_CHENHLECH := VAR_CHENHLECH - CUR_LOTHUOC.SOLUONGTON;
        ...
        ELSE
          VAR_SOTHUOCDALAY := VAR_SOTHUOCDALAY + VAR_CHENHLECH;
  
          UPDATE LOTHUOC 
          SET SOLUONGTON = SOLUONGTON - VAR_CHENHLECH 
          WHERE MALOTHUOC = CUR_LOTHUOC.MALOTHUOC;
    ...
    IF VAR_CHENHLECH > 0 THEN
      RAISE_APPLICATION_ERROR(-20001, 'Số lượng thuốc vượt quá số lượng tồn của tất cả các lô trong kho.');
    END IF;

  ELSE

    VAR_CAPNHAT := ABS(VAR_CHENHLECH);
    FOR CUR_LOTHUOC IN (SELECT MALOTHUOC, SOLUONGTON, SOLUONGNHAP 
                        FROM LOTHUOC 
                        WHERE MATHUOC = :NEW.MATHUOC
                        ORDER BY HANSD DESC) 
    LOOP
        EXIT WHEN VAR_CAPNHAT = 0;
        
        IF CUR_LOTHUOC.SOLUONGTON <> CUR_LOTHUOC.SOLUONGNHAP THEN
            
            IF VAR_CAPNHAT >= CUR_LOTHUOC.SOLUONGNHAP - CUR_LOTHUOC.SOLUONGTON THEN
                VAR_CAPNHAT := VAR_CAPNHAT - (CUR_LOTHUOC.SOLUONGNHAP - CUR_LOTHUOC.SOLUONGTON);
                
                UPDATE LOTHUOC 
                SET SOLUONGTON = CUR_LOTHUOC.SOLUONGNHAP 
                WHERE MALOTHUOC = CUR_LOTHUOC.MALOTHUOC;
            ELSE
                UPDATE LOTHUOC 
                SET SOLUONGTON = CUR_LOTHUOC.SOLUONGTON + VAR_CAPNHAT 
                WHERE MALOTHUOC = CUR_LOTHUOC.MALOTHUOC;
...
AS
  TYPE PK_RECORD IS RECORD (
    MAPK PHIEUKHAM.MAPK%TYPE,
    MACC BENHNHAN.CCCD%TYPE,
    HOTEN BENHNHAN.HOTEN%TYPE,
    GIOITINH BENHNHAN.GIOITINH%TYPE,
    NGAYSINH BENHNHAN.NGAYSINH%TYPE,
    SDT BENHNHAN.DIACHI%TYPE,
    STT BENHNHAN.TIENSUBENH%TYPE,
    DIACHI BENHNHAN.DIACHI%TYPE,
    HOTENBACSI BENHNHAN.DIUNG%TYPE,
    TRANGTHAI BENHNHAN.DIUNG%TYPE,
    TIENSUBENH BENHNHAN.TIENSUBENH%TYPE,
    DIUNG BENHNHAN.DIUNG%TYPE
  );
  
  PK_REC PK_RECORD;
  
  CURSOR CUR_MAPK IS
  SELECT P.MAPK, B.CCCD, B.HOTEN, B.GIOITINH, B.NGAYSINH, B.SDT, P.STT, B.DIACHI, B1.HOTEN, P.TRANGTHAITH, B.TIENSUBENH, B.DIUNG
  FROM PHIEUKHAM p
  JOIN BENHNHAN b ON P.MABN = B.MABN
  JOIN BACSI b1 ON P.MABSC = B1.MABS
  WHERE EXTRACT(DAY FROM P.NGAYKHAM) = PAR_DAY 
  AND EXTRACT(MONTH FROM P.NGAYKHAM) = PAR_MONTH 
  AND EXTRACT(YEAR FROM P.NGAYKHAM) = PAR_YEAR;
BEGIN

  DBMS_OUTPUT.PUT_LINE(N'Thông tin các bệnh nhân khám trong ngày '|| PAR_DAY ||'-'|| PAR_MONTH ||'-'|| PAR_YEAR ||':');

  OPEN CUR_MAPK;
  FETCH CUR_MAPK INTO PK_REC;
  WHILE CUR_MAPK%FOUND LOOP
    
    DBMS_OUTPUT.PUT_LINE(N'Họ và tên: '|| PK_REC.HOTEN);
    DBMS_OUTPUT.PUT_LINE(N'Số căn cước: '|| PK_REC.MACC);
    DBMS_OUTPUT.PUT_LINE(N'Địa chỉ: '|| PK_REC.DIACHI);
    DBMS_OUTPUT.PUT_LINE(N'Ngày sinh: '|| PK_REC.NGAYSINH);
    DBMS_OUTPUT.PUT_LINE(N'Số điện thoại: '|| PK_REC.SDT);
    DBMS_OUTPUT.PUT_LINE(N'Giới tính: '|| PK_REC.GIOITINH);
    DBMS_OUTPUT.PUT_LINE(N'Tiền sử bệnh: '|| PK_REC.TIENSUBENH);
    DBMS_OUTPUT.PUT_LINE(N'Dị ứng: '|| PK_REC.DIUNG);
    DBMS_OUTPUT.PUT_LINE(N'Bác sĩ khám: '|| PK_REC.HOTENBACSI);
    DBMS_OUTPUT.PUT_LINE(N'Trạng thái: '|| PK_REC.TRANGTHAI);
    DBMS_OUTPUT.PUT_LINE(N'=============================================================');
...
EXCEPTION
  WHEN NO_DATA_FOUND THEN
    DBMS_OUTPUT.PUT_LINE('Không có dữ liệu cho ngày ' || PAR_DAY || '-' || PAR_MONTH || '-' || PAR_YEAR);

END;

SELECT * FROM PHIEUKHAM
SELECT * FROM BENHNHAN
EXEC HIENTHIDANHSACHDANGKYKHAM(19, 4, 2024);
EXEC INSERT_PHIEUKHAM_HOADON(2, 2, 2, 1, TO_DATE('19-04-2024', 'DD/MM/YYYY'), TO_DATE('17-04-2024', 'DD/MM/YYYY'), 'OPEN', 1,102,22,22,'DAU BUNG','DAU BUNG', 'BINH THUONG', 'CHUA CO KET LUAN');
CREATE SEQUENCE SEQ11_MAPK START WITH 17;
...
BEGIN
  UPDATE BACSI
  SET HOTEN = PAR_HOTEN, CCCD = PAR_CCCD, TRINHDO = PAR_TRINHDO, NGAYSINH = PAR_NGAYSINH, GIOITINH = PAR_GIOITINH, DIACHI = PAR_DIACHI, CHUYENKHOA = PAR_CHUYENKHOA
  WHERE MABS = PAR_MABS;
  COMMIT;
END;
...
CREATE OR REPLACE PROCEDURE INSERT_LOTHUOC (
  PAR_MATHUOC LOTHUOC.MATHUOC%TYPE, PAR_NHACC LOTHUOC.NHACC%TYPE, PAR_SOLUONGTON LOTHUOC.SOLUONGTON%TYPE,
  PAR_SOLUONGNHAP LOTHUOC.SOLUONGNHAP%TYPE, PAR_HANSD LOTHUOC.HANSD%TYPE, PAR_GIANHAP LOTHUOC.GIANHAP%TYPE, 
  PAR_GIABAN LOTHUOC.GIABAN%TYPE, PAR_NGAYNHAP LOTHUOC.NGAYNHAP%TYPE)

AS
BEGIN

  INSERT INTO LOTHUOC (MALOTHUOC, MATHUOC, NHACC, SOLUONGTON, SOLUONGNHAP, HANSD, GIANHAP, NGAYNHAP, GIABAN)
  VALUES (SEQ14_MALT.NEXTVAL, PAR_MATHUOC, PAR_NHACC, PAR_SOLUONGTON, PAR_SOLUONGNHAP, PAR_HANSD, PAR_GIANHAP, PAR_NGAYNHAP, PAR_GIABAN);
...
CREATE OR REPLACE PROCEDURE UPDATE_LOTHUOC (
  ...
  PAR_SOLUONGTON LOTHUOC.SOLUONGTON%TYPE, PAR_SOLUONGNHAP LOTHUOC.SOLUONGNHAP%TYPE, PAR_HANSD LOTHUOC.HANSD%TYPE,
...
BEGIN
  ...
  SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
  UPDATE LOTHUOC
  SET MATHUOC = PAR_MATHUOC, NHACC = PAR_NHACC, SOLUONGTON = PAR_SOLUONGTON, SOLUONGNHAP = PAR_SOLUONGNHAP,
  HANSD = PAR_HANSD, GIANHAP = PAR_GIANHAP, GIABAN = PAR_GIABAN, NGAYNHAP = PAR_NGAYNHAP
...
CREATE OR REPLACE PROCEDURE INSERT_CTDT (PAR_MADT CTDT.MADT%TYPE, PAR_MATHUOC CTDT.MATHUOC%TYPE,
  ...
  PAR_SOLUONGTHUOC CTDT.SOLUONGTHUOC%TYPE, PAR_GHICHU CTDT.GHICHU%TYPE)
AS
BEGIN
  INSERT INTO CTDT (MACTDT, MADT, MATHUOC, SOLANUONG, SOLUONGUONG, TRANGTHAIDATLICH, SOLUONGTHUOC, GHICHU)
  VALUES (SEQ16_MACTDT.NEXTVAL, PAR_MADT, PAR_MATHUOC, PAR_SOLANUONG, PAR_SOLUONGUONG, PAR_TRANGTHAI, PAR_SOLUONGTHUOC, PAR_GHICHU);
